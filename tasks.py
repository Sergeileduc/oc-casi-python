...
def build(c):
    ...
    c.run("pyinstaller --windowed --icon owncloud.ico --clean --log-level=WARN oc-casi.py")  # noqa:E501
    path = Path(__file__).parent
    src = path / "config.sample.json"
    dist = path / "dist"
    dst = dist / "oc-casi" / "config.json"
    print("copy dist config.json")
    shutil.copy(src, dst)
    print("making a zip")
    shutil.make_archive(dist / "oc-casi", "zip", dist, "oc-casi")
